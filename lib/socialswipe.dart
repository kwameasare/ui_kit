...
class SocialSwipe extends StatefulWidget {
  @override
  _SocialSwipeState createState() => _SocialSwipeState();
}

class _SocialSwipeState extends State<SocialSwipe>
  ...
  void _hDragUpdate(DragUpdateDetails details) {
    final cardDrag =
        (details.globalPosition.dx - startDrag.dx) / context.size.width;

    setState(() {
      scrollPercent = (startDragPercentScroll + (-cardDrag / userList.length))
  ...
  void _hDragEnd(DragEndDetails details) {
    ...
    singleCardEnd = (scrollPercent * userList.length).round() / userList.length;
